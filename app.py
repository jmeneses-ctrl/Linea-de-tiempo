...
try:
    ...
    if xl_file is None:
        ...
        with tab2:
            ...
            # ----------------------------------------------------
            # 🚨 PARCHE DE CÁLCULO MEJORADO 🚨
            # Lógica: Si Manual existe -> Usar Manual. Si no -> Usar Proyectada.
            # ----------------------------------------------------
            
            # 1. Asegurar tipos
            cols_date = ["Fecha_Real_Manual", "Fecha_Vigente", "Fecha_Proyectada"]
            for col in cols_date:
                if col in df_edit.columns:
                    df_edit[col] = pd.to_datetime(df_edit[col], errors='coerce')

            # 2. Lógica de Relleno Solicitada: Manual > Proyectada
            # Paso A: Prioridad 1 = Fecha Manual
            # Si hay manual, usamos manual. Si no, mantenemos lo que haya en Vigente (por si acaso Excel traía algo)
            df_edit['Fecha_Vigente'] = df_edit['Fecha_Real_Manual'].combine_first(df_edit['Fecha_Vigente'])
            
            # Paso B: Prioridad 2 (Fallback) = Fecha Proyectada
            # Si después del paso A sigue vacía (o si Excel traía vacío y no había manual), usamos Proyectada.
            if 'Fecha_Proyectada' in df_edit.columns:
                 df_edit['Fecha_Vigente'] = df_edit['Fecha_Vigente'].combine_first(df_edit['Fecha_Proyectada'])
            ...
            # --- DEFINIR COLUMNAS ---
            # OJO: Fecha Proyectada se usa para cálculo pero no se muestra
            cols_deseadas = [
                "Estado", 
